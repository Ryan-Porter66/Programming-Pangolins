        ...
        public static void createPayStubsPDF(EmployeeList empList)
        {
            ...
            try
            {
                ...
                foreach (Employee emp in empList.Employees)
                {
                    ...
                    {
                        SalaryEmployee salaryEmployee = (SalaryEmployee)emp;
                        printCellToTable("1", payrollTable);
                        printCellToTable(salaryEmployee.SalaryPerPayPeriod.ToString(), payrollTable);
                    }
                    else if (emp is HourlyEmployee)
                    {
                        HourlyEmployee hourlyEmployee = (HourlyEmployee)emp;
                        printCellToTable(hourlyEmployee.HoursWorked.ToString(), payrollTable);
                        printCellToTable(hourlyEmployee.PayPerHour.ToString(), payrollTable);
                    }

                    printCellToTable("--------", payrollTable);
